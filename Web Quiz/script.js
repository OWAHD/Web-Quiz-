...
const list_of_Questions = [
  "Which of the following equality operators also checks type?",
  "(=)",
  "(!==)",
  "(===)",
  "(==)",
  "If we have an array that contains 5 items, which index number corresponds to the last item?",
  "0",
  "-L",
  "5",
  "4",
  "When passing the function named 'thisFunction' as an argument into 'thatFunction', which syntax calling 'thatFunction' is correct?",
  "thisFunction(thatfunction)",
  "thatFunction(function thisFunction)",
  "function(thisFunction, thatFunction)",
  "thatFunction(thisFunction)",
  "Using JQuery, which of the following methods searches for an element by the ID name 'testID' ?",
  "document.getElementByID('testID')",
  "getElementID('testID')",
  "$('testID')",
  "$('id', 'testID')",
  "What is the form of data pushed into localStorage that is stored as key/value pairs?",
  "Array",
  "String",
  "Map",
  "Object",
  "During propogation of events through the DOM, what is the name of the phase that travels from the TARGET to the ROOT?",
  "Bubbling",
  "Cascading",
  "Capturing",
  "Triggering",
  "What is the name of the notation for transmitting data between servers and webpages",
  "JSON",
  "ASCII",
  "Hexadecimal",
  "2Bit",
...
var secondsLeft = 5460; //note, in deci-seconds, not mili seconds
...
function Begin() {
  ...
  document
    .querySelector(".hidden_Main")
    .setAttribute("class", "not_Hidden_Main");
  document
    .querySelector(".hidden_List")
    .setAttribute("class", "not_Hidden_List");
  document
    .querySelector(".hidden_Score")
    .setAttribute("class", "not_Hidden_Score");
...
function check_Answer() {
  ...
  if (answer == answer_key[question_count]) {
    // Answer IS correct
    console.log(
      "You're correct! Answered: " +
        answer +
        " and the correct answer was: " +
        answer_key[question_count]
    );
    correct_Count++;
...
function end_Of_Quiz() {
  ...
  if (secondsLeft == 0) {
    var first_Name = prompt(
      ...
      "Enter your name here for high scores!"
    );
  } else {
    var first_Name = prompt(
      "You've ended the quiz with " +
  ...
  document
    .querySelector(".not_Hidden_Score")
    .setAttribute("class", "hidden_Score");
  ...
  document
    .querySelector(".not_Hidden_Main")
    .setAttribute("class", "final_Screen_Header");
...
function display_HighScores() {
  ...
  for (let i = 0; i < initials.length; i++) {
    if (i == high_score_position) {
      ...
      document.body.lastChild.lastChild.textContent =
        initials[i] + " - - - " + init_scores[i];
...
function Timer() {
  ...
  var timerInterval = setInterval(function () {
    ...
    if (document.querySelector(".not_Hidden_Score") != null) {
      document.querySelector(".not_Hidden_Score").textContent =
        Math.floor(secondsLeft / 10) + " seconds left on the quiz.";
    }
